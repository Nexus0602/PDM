...
import es.loyola.inftv.app.services.ListadoMarcasServlet;

public class TestListadoMarcas {
	...
	public void testGetProductos() throws IOException, ServletException {
		...
		new ListadoMarcasServlet().doGet(request,response);
		...
		if(objResponse.has("code") && objResponse.has("resultado") && objResponse.has("mensaje") ) {
			...
			assertTrue( "El array no tiene un elemento", arrayRes.length() == MarcaManager.getListadoMarcas().size());
