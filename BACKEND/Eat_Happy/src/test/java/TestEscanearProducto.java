...
import es.loyola.inftv.app.services.EscanearProductoServlet;



public class TestEscanearProducto {
	...
	public void testDoGet() throws IOException,ServletException{
		...
		when(request.getParameter("cb")).thenReturn("A23254");
		...
		new EscanearProductoServlet().doGet(request,response);
		...
		if(objresponse.has("code") && objresponse.has("mensaje")&& objresponse.has("resultado"))
		{
			...
			assertTrue("El codigo devulto no es ok", arrayResultado.length()==3);
