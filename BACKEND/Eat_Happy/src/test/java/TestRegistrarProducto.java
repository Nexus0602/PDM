...
import es.loyola.inftv.app.services.RegistrarProductoServlet;
...
/*
	...
	public void testDoPost() throws IOException,ServletException{
		...
		when(request.getParameter("producto")).thenReturn("");
		...
		new RegistrarProductoServlet().doPost(request,response);
		...
		if(objresponse.has("code") && objresponse.has("message")&& objresponse.has("result"))
		{
			...
			assertTrue("El mensaje devuelto no es ok",objresponse.get("message").equals("ok"));
			JSONArray arrayResultado=objresponse.getJSONArray("result");
